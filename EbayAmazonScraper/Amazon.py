...
# Function that returns products
def get_data(data, headers, page=1, keyword="Ipad"):
    # Add keyword and page number to URL
    ...
    for item in soup.select('.s-result-item'):
        ...
        # using CSS Selector names on Amazon's search page

        # Finds title of product
        try:
        ...
        # If title is not found, it will be found in below CSS Selector
        ...
        # Finds review count of product
        try:
            review_count = item.find("span", attrs={'class': 'a-size-base'}).string.strip()

        # If review counts are not found, reviews will be listed as unavailable.
        except AttributeError:
            review_count = "Reviews Unavailable"

        # Finds URL of product
        try:
            url = item.find('a', {'class': 'a-link-normal s-no-outline'}).get('href')
            # Adds the Amazon URL prefix as its just product URL which is returned.
...
multi_get_data(data, headers, start_page=1, end_page=8)
print(json.dumps(data))

# Below code used for testing purposes, as well as start_time above multi_get_data
